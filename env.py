...
class ArmEnv(object):
    ...
    dt = 0.1    # 轉動的速度跟dt有關
    action_bound = [-1, 1] # 轉動的範圍
    goal = {'x': 100., 'y': 100., 'l': 40} # Goal的座標和長度
    state_dim = 2 # State space
    action_dim = 2 # Action space

    def __init__(self):
        # 記錄手臂資訊
        self.arm_info = np.zeros(2, dtype=[('l', np.float32), ('r', np.float32)])
        self.arm_info['l'] = 100 # 手臂長度
        self.arm_info['r'] = np.pi/6 # 手臂的端點角度
